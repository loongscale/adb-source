...
/**	从ADB_TRACE环境变量中读取一个用逗号、空格、colum、semi-column分隔的标签列表
 */
...
int adb_main(int is_daemon, int server_port)
{
    ...
    atexit(adb_cleanup);	// 程序退出的时候调用adb_cleanup()函数。
#ifdef HAVE_WIN32_PROC
...
{
#if ADB_HOST
    adb_sysdeps_init();	// 初始化了一些临界区。
    adb_trace_init();	// 初始化跟踪标记。
