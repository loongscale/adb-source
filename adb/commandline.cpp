...
void help()
{
    ...
    fprintf(stderr,
        ...
        "\n"
        "device commands:\n"
        "  adb push <local> <remote>    - 拷贝文件或目录到设备\n"
        "  adb pull <remote> [<local>]  - 从设备拷贝文件或目录\n"
        "  adb sync [ <directory> ]     - copy host->device only if changed\n"
        "                                 (-l means list but don't copy)\n"
        "                                 (see 'adb help all')\n"
        "  adb shell                    - 运行远程 shell 交互\n"
        "  adb shell <命令>          - 运行远程 shell 命令\n"
        "  adb emu <命令>            - 运行模拟器控制台命令\n"
        "  adb logcat [ <filter-spec> ] - 查看设备日志\n"
        "  adb forward --list           - 列出所有转发的 socket 连接。\n"
        "                                 下面是列表的格式：\n"
        "                                    <serial> \" \" <local> \" \" <remote> \"\\n\"\n"
        "  adb forward <local> <remote> - 转发 socket 连接\n"
        "                                 转发规范是下列之一: \n"
        "                                   tcp:<端口>\n"
...
static void *stdin_read_thread(void *x)
{
    ...
    for(;;) {
        ...
        r = unix_read(fdi, buf, 1024);	// 读取输入的命令。
...
int adb_download(const char *service, const char *fn, unsigned progress)
{
    ...
    status = adb_download_buffer(service, fn, data, sz, progress);
    free(data);
    return status;
}
...
int adb_commandline(int argc, char **argv)
{
	...
	/* 如果定义了环境变量"ANDROID_PRODUCT_OUT"，它应该是一个绝对路径，包含特定产品在所有系统中的映像。
	 * 如果未定义，并且adb命令行请求这个信息，那么用户必须通过"-p"指定它的路径。
	 */
    ...
    if (server_port_str && strlen(server_port_str) > 0) {
        ...
        if (server_port <= 0 || server_port > 65535) {
            fprintf(stderr,
                    "adb: 环境变量 ANDROID_ADB_SERVER_PORT 必须是一个正数并小于65535：\"%s\"\n",
                    server_port_str);
            return usage();
        }
    }
    ...
    while(argc > 0) {
        if(!strcmp(argv[0],"server")) {
			...
			/* 这是一个特殊的标识，仅用于当 ADB 客户端启动 ADB 服务端时。 */
    ...
    adb_set_transport(ttype, serial);	// 设置传输。
    adb_set_tcp_specifics(server_port);	// 设置服务器端口。
...
top:
    ...
    if(!strcmp(argv[0], "devices")) {
        ...
        if (argc < 2)
            ...
            fprintf(stderr, "用法：adb devices [-l]\n");
            return 1;
        }
    ...
    if(!strcmp(argv[0], "connect")) {
        char *tmp;
        if (argc != 2) {
            fprintf(stderr, "用法：adb connect <host>[:<port>]\n");
            return 1;
        }
    ...
    if(!strcmp(argv[0], "disconnect")) {
        char *tmp;
        if (argc > 2) {
            fprintf(stderr, "用法：adb disconnect [<host>[:<port>]]\n");
            return 1;
        }
        if (argc == 2) {
    ...
    if(!strcmp(argv[0], "forward")) {
        ...
        else
        {
          ...
          if (argc != 3)
            return usage();
          command = no_rebind ? "forward:norebind:" : "forward";
