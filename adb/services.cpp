...
void connect_emulator(char* port_spec, char* buffer, int buffer_size)
{
	...
	port_separator = strchr(port_spec, ',');
    if (!port_separator) {
    ...
    *port_separator++ = 0;
    console_port = strtol(port_spec, NULL, 0);
    adb_port = strtol(port_separator, NULL, 0);
    ...
    candidate_slot = get_available_local_transport_index();
    if (candidate_slot < 0) {
