...
int adb_connect(const char *service)
{
    ...
    if(fd == -2 && __adb_server_name) {
        ...
        fprintf(stdout,"* 守护进程未启动。 正在启动它，然后监听 %d 端口 *\n",
                __adb_server_port);
    start_server:
        if(launch_server(__adb_server_port)) {
            fprintf(stderr,"* 守护进程启动失败 *\n");
            return -1;
        } else {
            fprintf(stdout,"* 守护进程启动成功 *\n");
        }
		/* 给服务端一些时间正确的启动并检测设备。 */
        adb_sleep_ms(3000);
        // fall through to _adb_connect
    } else {
		// 如果服务端正在运行，检查它的版本确保它不是过时的。
        char buf[100];
        size_t n;
